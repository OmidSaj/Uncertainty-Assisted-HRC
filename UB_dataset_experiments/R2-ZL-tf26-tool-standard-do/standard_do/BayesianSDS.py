...
from keras.layers import Input, Dropout,ZeroPadding2D,Cropping2D
...
def BaySDS_fit(model,X_train,Y_train,X_val,Y_val,verbose,
    ...
    model.compile(loss='categorical_crossentropy', optimizer=opt_method, metrics=["accuracy"],sample_weight_mode="temporal")
    
    model_callbacks = [EarlyStopping(monitor='val_loss', patience=patience),
             ModelCheckpoint(filepath=model_filename_save, monitor='val_loss', save_best_only=True,mode='min'),lrate,plot_losses]  #plot_losses
